...
SANS les fonctionnalités d'email qui ne marchent pas
"""
...
structure = [
    ...
    ('  ├── 📄 config.php', 'Configuration (emails désactivés)'),
    ...
    ('  │   └── email_logs.json', 'Logs (non fonctionnel)'),
]
...
landing_sections = [
    ...
    ('Galerie Images', 'Carrousels des installations et chambres'),
...
billing_endpoints = [
    ...
    ['POST', 'api/send_invoice.php', 'Envoi email (NON FONCTIONNEL)'],
]
...
data_files = [
    ...
    ('email_logs.json', 'Historique d\'envoi d\'emails (NON FONCTIONNEL)'),
...
deployment = [
    ...
    'Configurer PHP 7.4+',
    'Vérifier la configuration de /data/ pour écriture',
...
add_colored_heading(doc, '8.4 Configuration d\'Emails (NON FONCTIONNEL)', 2)
doc.add_paragraph(
    'Une interface de configuration SMTP existe dans le fichier config.php, '
    'mais la fonctionnalité d\'envoi d\'emails ne fonctionne actuellement pas. '
    'Cette section reste disponible pour une implémentation future:'
)
...
# ===== 10. FONCTIONNALITÉS NON FONCTIONNELLES =====
doc.add_page_break()
add_colored_heading(doc, '10. FONCTIONNALITÉS NON FONCTIONNELLES', 1)

add_colored_heading(doc, '10.1 Envoi d\'Emails (Non Implémenté)', 2)
doc.add_paragraph(
    'L\'interface de configuration SMTP existe mais la fonctionnalité d\'envoi d\'emails '
    'n\'est pas implémentée. Les éléments concernés:'
)

non_functional = [
    'Envoi de mail de confirmation de réservation',
    'Envoi de mail de validation par l\'administrateur',
...
doc.add_paragraph(
    'Ces fonctionnalités pourront être implémentées ultérieurement en configurant '
    'correctement un serveur SMTP et en activant les appels API correspondants.'
)

# ===== 11. EXEMPLES DE DONNÉES =====
doc.add_page_break()
add_colored_heading(doc, '11. EXEMPLES DE DONNÉES', 1)

add_colored_heading(doc, '11.1 Structure d\'un Utilisateur', 2)
...
add_colored_heading(doc, '11.2 Détail d\'une Facture', 2)
...
# ===== 12. RECOMMANDATIONS =====
doc.add_page_break()
add_colored_heading(doc, '12. RECOMMANDATIONS ET AMÉLIORATIONS FUTURES', 1)

add_colored_heading(doc, '12.1 Sécurité - Priorité Haute', 2)
security_rec = [
...
add_colored_heading(doc, '12.2 Emails - Priorité Haute', 2)
email_rec = [
    'Implémenter correctement l\'envoi d\'emails SMTP',
    'Ajouter des templates d\'emails HTML professionnels',
    'Envoyer confirmations de réservation',
    'Envoyer notifications de validation/rejet',
    'Envoyer factures par email',
    'Ajouter gestion des erreurs d\'envoi',
]
for rec in email_rec:
    doc.add_paragraph(rec, style='List Bullet')

add_colored_heading(doc, '12.3 Performance - Priorité Moyenne', 2)
perf_rec = [
...
add_colored_heading(doc, '12.4 Fonctionnalités - Priorité Moyenne', 2)
feature_rec = [
...
add_colored_heading(doc, '12.5 DevOps - Priorité Basse', 2)
devops_rec = [
...
# ===== CONCLUSION =====
doc.add_page_break()
add_colored_heading(doc, '13. CONCLUSION', 1)
...
doc.add_paragraph()
doc.add_paragraph(
    'Note: L\'implémentation de l\'envoi d\'emails est recommandée comme '
    'prochaine étape pour améliorer la communication avec les clients.'
)
...
run = footer_para.add_run(
    ...
    'NOTA: Fonctionnalité emails non implémentée'
)
run.font.size = Pt(9)
run.font.color.rgb = RGBColor(138, 155, 176)
...
print('✓ Sections d\'emails marquées comme NON FONCTIONNELLES')
print('✓ Le rapport comprend maintenant une section dédiée aux fonctionnalités non implémentées')
